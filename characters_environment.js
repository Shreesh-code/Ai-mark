...
function staaaaaaaaaghrt(){
stats="s";
document.getElementById("other_h3").innerHTML="game is has start";
}
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("pres to start the button", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(stats=="s" && nx!="" && gameConfig.status==="start") {
    world_start.play();
